...
object SelfCheckoutGUI extends JFXApp {
  ...
  var myButton:Button=new Button("click me")
  var penny:CheckBox=new CheckBox("Penny Arcade")
/*
 ...
 */
    ...
    myButton.addEventFilter(MouseEvent.MOUSE_CLICKED, (_: MouseEvent) => {
      updateDisplay()
    })
    scanField.addEventFilter(MouseEvent.MOUSE_CLICKED, (_: MouseEvent) => {
      scanField.text.value = "clicked text field"
    })
    /*
...
*/
    scanField.text.value = "clicked mouse"
    var page:ComicPage=new ComicPage("test1.html")
    if (penny.selected()) {
      page.addPennyArcade()
    }
    page.writePage()
